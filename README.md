...
0으로 초기화한다. 그 단계에서는 구간 이탈 확률의 수치 상한과 전체 seeded 흐름을 후속 의무로 남겼다.
<!-- gaussian-payload:overview:end -->

<!-- hyperball-tail:overview:start -->
후속 [iid 제곱합 꼬리 경계](haetae-1.2.0-easycrypt/docs/hyperball-iid-tail-bound.md)는
앞 단계의 정확한 S 분포에 수치 상계를 추가한다. 실제 승인 raw 제곱의 지수
모멘트를 증명해, 모드 2·3·5의 충분한 안전 구간 이탈 확률을 각각
**2^-29·2^-44·2^-54 미만**으로 제한했다. 같은 상계가 실제 Gaussian·수치 helper를
합성한 한 번의 iid 시도에서 **승인하면서 반환 계수의 정수 제곱 노름이 모드 경계를
넘는 결합 사건**에도 적용된다. 승인 후 조건부 확률이나 외부 retry 전체의 경계는 아니다.
명시적 균등 바이트와 기존 초기화 모형을 유지하며, concrete SHAKE·전체 seeded 실행은 별도다.
