...
half-length is $\ell = 2^{7-s}$ and the \emph{twiddle factor} for group $k$ is
$\zeta_k^{(s)} = \zeta^{\brev_8(k) \cdot \ell}$ where $k$ ranges over the
$2^s$ groups.  The butterfly operation on a pair $(a, b)$ with twiddle factor
$w$ is:
\begin{equation}
...
\begin{algorithm}[h]
\caption{Montgomery Reduction REDC$(a, q, R, q')$}
...
\If{$t \ge q$} \Return $t - q$ \Else\ \Return $t$ \EndIf
\end{algorithmic}
\end{algorithm}

In the \haetae{} implementation, $R = 2^{32}$, $q = 64513$,
$q' = \mathsf{QINV} = 940508161$ ($\equiv -q^{-1} \bmod 2^{32}$), and
$\mathsf{MONT} = R \bmod q = 14321$.
...
$\abs{a} < q \cdot 2^{15}$:
\begin{equation}
  \text{int32}(a_{\text{lo}} \cdot q') \cdot q \;\text{(as signed 64-bit)} +\; a
  \;\text{ shifted right by 32}
\label{eq:signed_mont}
\end{equation}
yields a value congruent to $a \cdot R^{-1} \pmod{q}$ and bounded by $q/2$.
...
extraction.  The extracted \easycrypt{} type for \jasm{ptr u32[256]} is the
\easycrypt{} type \ec{W32.t Array256.t}, i.e., an array of 256 machine words.
...
Cooley--Tukey NTT over the same ring structure but with the \emph{different}
exponent formula $\brev_8((s \bmod \ell) \cdot 2)$.  Our work adapts
