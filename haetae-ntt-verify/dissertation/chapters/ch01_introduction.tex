...
\begin{aligned}
  \text{Jasmin}
    &\xrightarrow{\text{extraction}} \text{EC module} \\
    &\xrightarrow{\text{pRHL equivalence}} \text{EC imperative spec} \\
    &\xrightarrow{\text{loop invariant}} \text{EC abstract spec}.
\end{aligned}
\]
...
\begin{description}
  \item[\textbf{C1 -- Montgomery correctness.}]
    A machine-checked proof of the Montgomery reducer: it computes
    $a \cdot R^{-1} \bmod q$ for all inputs in the relevant working range,
    with $R = 2^{32}$, $q = 64513$, verified bit-width bounds, and
    signed-integer arithmetic lemmas absent from the \easycrypt{} standard
  ...
  \item[\textbf{C3 -- Abstract spectral specification.}]
    An algebraic library establishing properties of the primitive root
    $\zeta_0 = 426$ in $\Zq$, bit-reversal permutation lemmas, and a
    butterfly-stage induction connecting the iterative algorithm to the
  ...
  \item[\textbf{C5 -- End-to-end correctness.}]
    Top-level forward and inverse correctness theorems relate the exported
    \jasmin{} procedures to their abstract specifications
    (\Cref{ch:end_to_end}).
\end{description}
