...
lemma fqmul_word_to_coeff_mul_bound_ph aa bb :
  phoare [Hpoly_extract.M.__fqmul :
    W32.to_sint a = aa /\ W32.to_sint b = bb /\
    - Fq.SignedReductions.R %/ 2 * Fq.q <= aa * bb <
      Fq.SignedReductions.R %/ 2 * Fq.q ==>
    NTT_Fq.word_to_coeff res = incoeff aa * incoeff bb * inv NTT_Fq.R /\
    Fq.bw32 res 16] = 1%r.
proof.
by conseq Fq.fqmul_ll (fqmul_word_to_coeff_mul_bound_h aa bb).
qed.
...
lemma jzetas_bound16 i :
  1 <= i < 256 =>
  Fq.bw32 (BArray1024.get32 Hpoly_extract.jzetas i) 16.
proof.
move=> hi.
rewrite /Hpoly_extract.jzetas BArray1024.get32_of_list32 1:/#.
...
lemma jzetas_inv_bound16 i :
  0 <= i < 256 =>
  Fq.bw32 (BArray1024.get32 Hpoly_extract.jzetas_inv i) 16.
proof.
move=> hi.
rewrite /Hpoly_extract.jzetas_inv BArray1024.get32_of_list32 1:/#.
...
lemma forward_twiddle_product_bound_from_inner
  ...
  1 <= zc < 256 =>
  - Fq.SignedReductions.R %/ 2 * Fq.q <=
    W32.to_sint (BArray1024.get32 Hpoly_extract.jzetas zc) *
...
exact (fqmul_product_bound_16_24
  (BArray1024.get32 Hpoly_extract.jzetas zc)
...
lemma inverse_twiddle_product_call_bound_from_inner
  ...
  zc \in range 0 255 =>
  - Fq.SignedReductions.R %/ 2 * Fq.q <=
    W32.to_sint (BArray1024.get32 Hpoly_extract.jzetas_inv zc) *
    W32.to_sint
      (BArray1024.get32 rp j - BArray1024.get32 rp (j + len)) <
    Fq.SignedReductions.R %/ 2 * Fq.q.
proof.
...
exact (fqmul_product_bound_16_24
  (BArray1024.get32 Hpoly_extract.jzetas_inv zc)
  (BArray1024.get32 rp j - BArray1024.get32 rp (j + len))
  hzbw hdiff24).
qed.
...
lemma inverse_final_product_bound_call_h
  ai bi coeff :
  hoare [Hpoly_extract.M.__fqmul :
    ai = BArray1024.get32 Hpoly_extract.jzetas_inv 255 /\
...
lemma inverse_final_product_bound_from_inv rp j :
  ...
  0 <= j < 256 =>
  - Fq.SignedReductions.R %/ 2 * Fq.q <=
    W32.to_sint (BArray1024.get32 Hpoly_extract.jzetas_inv 255) *
...
exact (fqmul_product_bound_16_24
  (BArray1024.get32 Hpoly_extract.jzetas_inv 255)
...
while (
  ...
  barray256_bound_by rp{1}
    (final_bound j{1}) /\
  zetasp{1} = Hpoly_extract.jzetas_inv /\
  zetas_inv{2} = NTT_Fq.zetas_inv /\
  j{1} = j{2} /\
  0 <= j{1} <= 256 /\
  zeta_0{1} = BArray1024.get32 Hpoly_extract.jzetas_inv 255
).
+ wp.
  sp.
...
+ wp.
  ...
  + wp.
    while (
      ...
      zeta_0{1} =
        BArray1024.get32 Hpoly_extract.jzetas_inv (zetasctr{1} - 1) /\
