...
\begin{document}
	\begin{tikzpicture}[
		...
		\foreach \x in {3,4,6,8,10,11,12}{
			\draw[line width=1pt, dashed, magenta] (\x, -4) -- (\x, 0.25);
		}
		
		\node[unbounded] (AutoGP) at (10,-1) {AutoG\&P\\ (self, SMT)}; % 15
		\node[bounded] (CertiCrypt) at (4,-2) {CertiCrypt\\ (Rocq)}; % 9
		\node[bounded] (CryptHOL) at (12,-1) {CryptHOL\\ (Isabelle)}; % 17
		\node[unbounded] (CryptoVerif) at (3,-1) {CryptoVerif\\ (self)}; % 8
		\node[bounded] (EasyCrypt) at (6,-2) {EasyCrypt\\ (self, SMT)}; % 11
		\node[unbounded] (F7) at (6,-1) {F7\\ (self, SMT)}; % 11
		\node[unbounded] (F) at (11,-3) {F*\\ (self, SMT)}; % 16
		\node[bounded] (FCF) at (10,-2) {FCF\\ (Rocq)}; % 15
		\node[unbounded] (ZooCrypt) at (8,-1) {ZooCrypt\\ (self, SMT)}; % 13
