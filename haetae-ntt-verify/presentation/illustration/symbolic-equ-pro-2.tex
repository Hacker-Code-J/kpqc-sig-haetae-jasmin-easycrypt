...
%		\draw[tr=CO](q0)--node[el,left=1pt]{$a$}(q1);
%		\draw[tr=CO](q0)--node[el,right=1pt]{$a$}(q2);
...
%		{\textit{(but not bisimilar: branching structure differs)}};
		...
		\draw[tr=CB](bp0)--node[el,left]{}(bp1);
		\draw[tr=CB](bp1)--node[el,left]{}(bp2);
		...
		\draw[tr=CO](bq0)--node[el,right]{}(bq1);
		\draw[tr=CO](bq1)--node[el,right]{}(bq2);
		
		%% bisimulation relation at each level
		\draw[rel](bp0)--node[el]{$\mathcal{R}_0$}(bq0);
		\draw[rel](bp1)--node[el]{$\mathcal{R}_1$}(bq1);
		\draw[rel](bp2)--node[el]{$\mathcal{R}_2$}(bq2);
...
%		{$P\approx_{\!o}Q$ \enspace for all substitutions $\sigma$};
