...
%		\draw[fill=yellow!15, rounded corners=4pt] (15,-3) rectangle (16,-2.5) node[below right] {Security only};
		...
		\node[unbounded] (AutoGP) at (10,-1) {AutoG\&P}; % 15
		\node[bounded] (CertiCrypt) at (4,-2) {CertiCrypt}; % 9
		\node[bounded] (CryptHOL) at (12,-1) {CryptHOL}; % 17
		\node[unbounded] (CryptoVerif) at (3,-1) {CryptoVerif}; % 8
		\node[bounded] (EasyCrypt) at (6,-2) {EasyCrypt}; % 11
		\node[unbounded] (F7) at (6,-1) {F7}; % 11
		\node[unbounded] (F) at (11,-2) {F*}; % 16
		\node[bounded] (FCF) at (10,-2) {FCF}; % 15
