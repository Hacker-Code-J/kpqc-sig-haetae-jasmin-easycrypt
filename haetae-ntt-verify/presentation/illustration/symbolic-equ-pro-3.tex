...
%		{\textit{(stronger than trace eq.; closed under name instantiation)}};
		...
		\node[font=\tiny,CT] at (\cx+1.85,-3.56){$\pi_1$};
		\draw[pja=CR](d2)--(\cx+5.45,-4.12);
		\node[font=\tiny,CR] at (\cx+4.55,-3.56){$\pi_2$};
		
		%% left projection  π₁(B)
		\node[font=\tiny\bfseries,CT] at (\cx+0.95,-3.75){$\pi_1(\mathcal{B})$};
		...
		\node[font=\tiny\bfseries,CR] at (\cx+5.45,-3.75){$\pi_2(\mathcal{B})$};
