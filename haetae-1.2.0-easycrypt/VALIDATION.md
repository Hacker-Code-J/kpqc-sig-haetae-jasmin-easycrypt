...
| NTT 외 명세·증명 | `verify-new`의 151개 파일 | EOF 확인을 포함한 새 통합 실행 모두 PASS |
...
전체 로컬 명세·증명 163개를 각 단계에서 주 검증 대상으로 검사했다. import만 된 증명의
본문이 자동으로 재검사된다고 가정하지 않았다. 새 코드 게이트는 `-no-eco`와
명시적 `Proofs:check`를 사용하며, 원래 NTT 지원 재검증도 `-no-eco`의 기본
강한 검사 모드로 개별 실행했다.

이번에는 부호 비트·독립성·대칭 목표·벡터 분포·실제 부호 적용과 그 합성을
위한 증명 파일 7개를 추가했다. `gsb_uniform256`은 32바이트가 정확히
256개의 독립 공정 비트를 만든다는 정리다. 전체 표본 배열과 제곱 워드 배열을
함께 관측해도 이 부호들과 독립임을 증명했다. 257번째 더미의 제곱 기여도
그 상태에 포함하며, 저장된 표본과 부호는 여전히 256개다.

`gsc_actual_correct`는 `gib_bounds` 아래 실제 helper를 호출하는 iid 버퍼 모형의
확률 1 종료와, 각 unsigned 크기에 부호를 붙인 256개 정수의 정확한 분포 법칙을
증명한다. 독립적인 목표 `sign(G)·floor((abs(G)+32768)/65536)`의 iid 목록과
통계적 거리는 <2^-31이다. 0 질량을 그대로 유지하고 ±32768은 각각 ±1이 된다.
초기 배열은 임의의 word 값이며, 충분한 접두 구간 전제를 추가하지 않는다.
이는 구체적 SHAKE 난수 성질이나 제곱 누적값 자체의 Gaussian 법칙은 아니다.

`HyperballSignedScale`은 실제 scaling·반올림 뒤 부호 연산의 정확한 W32 표현을
증명한다. signed32 정수로 읽는 스칼라·벡터 정리는 반올림 크기≤2147483647
조건을 유지한다. 국소 부호 창과 전역 표본 인덱스를 연결하는 정리에는
`sample_offset=8*sign_offset`를 명시한다. 이상적 실수 scaling이나 모든
Hyperball 상태의 fit를 증명했다고 주장하지 않는다.

변경 없는 기존 비NTT 144개를 먼저 새로 검사하고, 새 7개는 소스 고정 후
같은 `verify-one.sh`로 검사했다. 최종 전체 목록과 소스 해시를 대조한 뒤
151개 PASS를 기록했다. 이전 156개 로컬 이론·증명과 생산 Jasmin·C·archive,
추출물 및 검증기는 변경하지 않았다. 새 공리나 수치 오라클도 없다.
검증기 회귀 18개는 모두 PASS이며 추출물 102개는 새 추출과 일치했다.
NTT 12개의 이전 검증 결과와 해시는 보존했으며 이번 재검사 수에는 포함하지 않았다.
구현 변경이 없어 이번 단계에서 KAT를 다시 실행하지 않았다.
[부호 출력 기록](docs/gaussian-signed-output.md)에 정확한 관측·전제·정리를 적었다.
...
현재 iid 모형의 초기 제곱 배열은 임의의 word 값이므로, 전체 반환 튜플 동치에서
정수 제곱합의 무오버플로 해석을 자동으로 얻지 않는다. 그 해석에는 이전
초기 limb·누적 범위 조건이 계속 필요하다. 새 확률 정리의 관측값은 저장되는
