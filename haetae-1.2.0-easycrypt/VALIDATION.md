...
| NTT 외 명세·증명 | `verify-new`의 113개 파일 | EOF 확인을 포함한 새 통합 실행 모두 PASS |
...
전체 로컬 명세·증명 125개를 각 단계에서 주 검증 대상으로 검사했다. import만 된 증명의
본문이 자동으로 재검사된다고 가정하지 않았다. 새 코드 게이트는 `-no-eco`와
명시적 `Proofs:check`를 사용하며, 원래 NTT 지원 재검증도 `-no-eco`의 기본
강한 검사 모드로 개별 실행했다.

이전 단계의 조건부분포 비교에 이어, 이번에는 7개 명세·증명 파일을 추가했다.
정수 Gaussian을 독립적인 무한 지수함수 합으로 정규화하고, 기존 이상적
조건부분포와 그 절댓값을 반올림한 분포가 정확히 같음을 증명했다. 실제 승인
표본 크기의 기존 통계적 거리<2^-39를 이 명시적인 Gaussian 목표로 이전했다.
113개 파일을 최종 통합 실행에서 각각 새로 검사했다. 실행 시간과 소스 해시는
`manifests/verification-results.json`에 기록했다. 이전 118개 로컬 이론·증명
...
식별로, 실제 구현의 후속 부호 처리나 거부 반복을 증명한 것은 아니다.
