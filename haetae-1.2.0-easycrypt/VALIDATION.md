...
| NTT 외 명세·증명 | `verify-new`의 90개 파일 | EOF 확인을 포함한 새 통합 실행 모두 PASS |
...
전체 로컬 명세·증명 102개를 각 단계에서 주 검증 대상으로 검사했다. import만 된 증명의
본문이 자동으로 재검사된다고 가정하지 않았다. 새 코드 게이트는 `-no-eco`와
명시적 `Proofs:check`를 사용하며, 원래 NTT 지원 재검증도 `-no-eco`의 기본
강한 검사 모드로 개별 실행했다.

이전 단계의 CDT·지수함수 수학적 오차와 Gaussian·Hyperball word 증명에 이어,
이번에는 11개 명세·증명 파일을 추가했다. 실제 제곱과 지수의 정수 의미,
반올림 전 Gaussian 수락식, 두 0 보정 규칙의 차이와 평균 승인 확률을 연결했다.
90개 파일을 최종 통합 실행에서 각각 새로 검사했다. 실행 시간과 소스 해시는
`manifests/verification-results.json`에 기록했다. 앞 단계의 파일 끝 빈 줄 정리도
이번 통합 실행에는 현재 소스 그대로 포함되며, 별도 followup 대체는 없다.
...
모든 고정 후보의 작은 오차로 과장하지 않고 예외를 명시했으며, 출력 분포·거부
