...
| NTT 외 명세·증명 | `verify-new`의 144개 파일 | EOF 확인을 포함한 새 통합 실행 모두 PASS |
...
전체 로컬 명세·증명 156개를 각 단계에서 주 검증 대상으로 검사했다. import만 된 증명의
본문이 자동으로 재검사된다고 가정하지 않았다. 새 코드 게이트는 `-no-eco`와
명시적 `Proofs:check`를 사용하며, 원래 NTT 지원 재검증도 `-no-eco`의 기본
강한 검사 모드로 개별 실행했다.

이번에는 균등 바이트와 전체 iid 버퍼 모형을 위한 명세·증명 파일 16개를 추가했다.
26바이트의 208비트 균등 입력이 기존 203비트 모형과 같은 관측 분포를 갖고,
미사용 5비트를 지워도 표본·두 제곱 limb·승인 워드가 모두 보존됨을 증명했다.
초기 49블록과 실제 sign-copy·carry·소비기 호출, 136바이트 재충전을 연결했다.

`gii_actual_correct`는 `gib_bounds`만 전제로 전체 iid 바이트 버퍼 모형의
확률 1 종료, 가시 256개 값의 정확한 독립 조건부분포 목록 법칙과 Gaussian 목표
거리<2^-31을 증명한다. 요청 256과 257을 모두 포함하며, 257번째 더미가
물리적 출력 슬롯에 저장된다고 주장하지 않는다. 초기 배열들은 임의의 word 값이다.
충분한 접두 구간이나 초기 제곱 워드의 정규형 전제를 추가하지 않았다.
이는 구체적인 SHAKE의 독립성·모든 seed의 종료 정리가 아니며, 부호 분포나
반환 표본·부호·제곱 배열 전체의 Gaussian 공동 법칙도 아니다.

변경 없는 기존 비NTT 128개를 먼저 새로 검사하고, 새 16개는 소스 고정 후
같은 `verify-one.sh`로 검사했다. 기존 이론은 새 파일에 의존하지 않는다.
전체 목록과 최종 해시를 대조한 뒤 144개 PASS를 기록했다. 실행 명령·시간과
해시는 `manifests/verification-results.json`에 있다. 이전 140개 로컬 이론·증명,
생산 Jasmin·C·archive·추출물과 검증기는 변경하지 않았다. 새 공리나 수치 오라클도 없다.
기존 NTT 12개의 앞선 개별 검증 결과와 해시를 보존했고, 추출물 102개는
현재 소스에서 새로 추출한 결과와 일치했다. NTT 재검사를 이번 단계에 포함한 것으로
기록하지 않는다. 구현 변경이 없어 이번 단계에서 KAT를 다시 실행하지 않았다.
...
256개 unsigned 크기 값이며, 부호의 독립성·적용은 별도 의무다.
