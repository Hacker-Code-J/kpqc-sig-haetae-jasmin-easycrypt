...
| NTT 외 명세·증명 | `verify-new`의 106개 파일 | EOF 확인을 포함한 새 통합 실행 모두 PASS |
...
전체 로컬 명세·증명 118개를 각 단계에서 주 검증 대상으로 검사했다. import만 된 증명의
본문이 자동으로 재검사된다고 가정하지 않았다. 새 코드 게이트는 `-no-eco`와
명시적 `Proofs:check`를 사용하며, 원래 NTT 지원 재검증도 `-no-eco`의 기본
강한 검사 모드로 개별 실행했다.

이전 단계의 모든 출력 집합에 대한 공동 확률에 이어, 이번에는 6개 명세·증명
파일을 추가했다. 실제·이상적 승인 확률의 양의 하한을 증명하고 각자의 native
조건부분포를 정규화하여 질량 1 및 통계적 거리<2^-39를 연결했다.
106개 파일을 최종 통합 실행에서 각각 새로 검사했다. 실행 시간과 소스 해시는
`manifests/verification-results.json`에 기록했다. 이전 112개 로컬 이론·증명
소스는 변경하지 않았으며, 새 결과에 미완성 증명이나 새 공리를 사용하지 않았다.
...
승인 후 조건부분포는 아래 정규화 정리로 연결했고, 부호와 거부 반복은 별도 의무다.
...
모두 해소한다. 목표는 기존 이상적 실험의 조건부분포이며, 별도의 반올림
Gaussian 확률 질량식과 같다는 식별은 포함하지 않는다.
...
이상적 실험에 승인 조건을 붙인 것이며, 별도 Gaussian 질량식 식별·부호·거부
반복·실제 SHAKE를 이번 결과로 주장하지 않음을 확인했다.
