...
require import ApiTarget SHAKEBlockSpec BArray40 BArray192 BArray200 Array5 Array24 Array25.
import SLH64.

theory SHAKEBlockCorrectness.
module Signer = ApiTarget.M(ApiTarget.Syscall).

(* The fixed-loop termination arguments below are adapted from the historical
   TargetKeygenKeccak1600 proof, and checked against the current ApiTarget. *)
lemma keccak_rho_offset_ll :
  islossless Signer.__keccakf1600_rho_offset.
proof.
proc.
while (0 <= t <= 24) (24 - t).
+ by move=> z; auto => />; smt().
...
lemma keccak_index_ll :
  islossless Signer.__keccakf1600_index.
proof. by proc; auto. qed.

lemma keccak_rho_ll :
  islossless Signer.__keccakf1600_rho.
proof.
proc.
...
lemma keccak_theta_sum_ll :
  islossless Signer.__keccak_theta_sum.
proof.
proc.
while (0 <= y <= 5) (5 - y).
+ move=> z.
...
lemma keccak_rol_u64_ll :
  islossless Signer.__rol_u64.
proof. by proc; auto. qed.

lemma keccak_theta_rol_ll :
  islossless Signer.__keccak_theta_rol.
proof.
proc.
while (0 <= x <= 5) (5 - x).
+ move=> z.
...
lemma keccak_rol_sum_ll :
  islossless Signer.__keccak_rol_sum.
proof.
proc.
while (0 <= x <= 5) (5 - x).
+ move=> z.
...
lemma keccak_andn_u64_ll :
  islossless Signer.__andn_u64.
proof. by proc; auto. qed.

lemma keccak_set_row_ll :
  islossless Signer.__keccak_set_row.
proof.
proc.
while (0 <= x <= 5) (5 - x).
+ move=> z.
...
lemma keccak_pround_ll :
  islossless Signer._keccak_pround.
proof.
proc.
while (0 <= y <= 5) (5 - y).
+ move=> z.
...
lemma keccak_statepermute_ll :
  islossless Signer.__keccakf1600_statepermute.
proof.
proc.
while (0 <= c <= 12) (12 - c).
+ move=> z.
...
lemma keccakf1600_ll :
  islossless Signer._keccakf1600.
proof.
proc.
...
lemma squeeze256_from_state_contract
    (out0 : BArray8192.t) (outoff0 : W64.t)
    (state0 : BArray200.t) (state_prop : BArray200.t -> bool) :
  hoare [Signer._keccakf1600 : sp_0 = state0 ==> state_prop res] =>
  hoare [Signer.__sample_full_squeeze256 :
    outp = out0 /\ outoff = outoff0 /\ sp_0 = state0 /\
    W64.to_uint outoff0 + 136 <= BArray8192.size
    ==>
    SHAKEBlockSpec.rate_block_matches
      res.`1 (W64.to_uint outoff0) res.`2 136 /\
    SHAKEBlockSpec.rate_block_frame
...
proc.
while (
  SHAKEBlockSpec.rate_prefix_matches
...
+ wp.
  ...
  + auto => /> &hr hprefix hframe hidx hlane
    ...
    have hi8 := SHAKEBlockSpec.div8_split
    ...
    have hwritecap :
      ...
      BArray8192.size by smt().
    have hp := SHAKEBlockSpec.rate_prefix_set_cursor
      outp{hr} (W64.to_uint outoff0) sp_0{hr}
      (W64.to_uint i{hr}) (W64.to_uint j{hr})
      hoff hi0 himod hjlt hwritecap hprefix.
    rewrite /SHAKEBlockSpec.rate_lane_byte in hp.
    split.
    ...
    + rewrite hidx.
      have -> :
        ...
        W64.to_uint outoff0 +
          (W64.to_uint i{hr} + W64.to_uint j{hr}) by ring.
      apply (SHAKEBlockSpec.rate_block_frame_set_inside
               out0 outp{hr} (W64.to_uint outoff0) 136
               (W64.to_uint i{hr} + W64.to_uint j{hr})
               (truncateu8
                 (SHAKEBlockSpec.drop_bytes
  ...
  + split.
    ...
    split.
    + rewrite SHAKEBlockSpec.drop_bytes0.
      rewrite W64.shr_div_le 1:/# /=.
    rewrite W64.ultE W64.of_uintK /= in hguard.
    exact hguard.
...
lemma squeeze256_ll :
  islossless Signer.__sample_full_squeeze256.
proof.
proc.
...
lemma squeeze256_returned_state_correct
    (out0 : BArray8192.t) (outoff0 : W64.t) (state0 : BArray200.t) :
  hoare [Signer.__sample_full_squeeze256 :
    outp = out0 /\ outoff = outoff0 /\ sp_0 = state0 /\
    W64.to_uint outoff0 + 136 <= BArray8192.size ==>
    (forall j, 0 <= j < 136 =>
      BArray8192.get8 res.`1 (W64.to_uint outoff0 + j) = BArray200.get8 res.`2 j) /\
    SHAKEBlockSpec.rate_block_frame out0 res.`1 (W64.to_uint outoff0) 136].
proof.
  have ht : hoare [Signer._keccakf1600 : sp_0 = state0 ==> true] by auto.
  conseq (squeeze256_from_state_contract out0 outoff0 state0 predT ht) => />.
  smt(SHAKEBlockSpec.rate_block_bytes).
qed.

lemma squeeze256_block_total
    (out0 : BArray8192.t) (outoff0 : W64.t) (state0 : BArray200.t) :
  phoare [Signer.__sample_full_squeeze256 :
    ...
    (forall j, 0 <= j < 136 =>
      BArray8192.get8 res.`1 (W64.to_uint outoff0 + j) = BArray200.get8 res.`2 j) /\
    SHAKEBlockSpec.rate_block_frame out0 res.`1 (W64.to_uint outoff0) 136] = 1%r.
proof.
  by conseq squeeze256_ll
    (squeeze256_returned_state_correct out0 outoff0 state0) => /> /#.
qed.

(* Refinement of the current Keccak helper to the local word specification. *)
...
lemma andn_u64_correct (a0 b0 : W64.t) :
  hoare [Signer.__andn_u64 :
    a = a0 /\ b = b0 ==>
    res = (invw a0) `&` b0].
proof. by proc; auto. qed.

lemma theta_sum_correct (a0 : BArray200.t) :
  hoare [Signer.__keccak_theta_sum :
    a = a0 ==>
...
lemma theta_rol_correct (c0 : BArray40.t) :
  hoare [Signer.__keccak_theta_rol :
    c = c0 ==>
...
lemma keccakf1600_rho_offset_correct (i0 : int) :
  hoare [Signer.__keccakf1600_rho_offset :
    0 <= i < 25 /\ i = i0 ==>
...
lemma keccakf1600_rho_correct (x0 y0 : int) :
  hoare [Signer.__keccakf1600_rho :
...
lemma target_rc_wordE (i : int) :
  0 <= i < 24 =>
  BArray192.get64 ApiTarget.haetae_keccak1600_rc i =
  rc_spec.[i].
proof.
move=> hi.
rewrite /ApiTarget.haetae_keccak1600_rc
...
lemma keccak_statepermute_word_correct (a0 : BArray200.t) :
  hoare [Signer.__keccakf1600_statepermute :
    a = a0 ==>
...
lemma keccakf1600_word_total (state0 : BArray200.t) :
  phoare [Signer._keccakf1600 : sp_0 = state0 ==>
    word_state_of_barray res = keccak_f1600 (word_state_of_barray state0)] = 1%r.
proof. by conseq keccakf1600_ll (keccakf1600_word_correct state0). qed.

lemma squeeze256_word_correct
    (out0 : BArray8192.t) (outoff0 : W64.t) (state0 : BArray200.t) :
  hoare [Signer.__sample_full_squeeze256 :
    outp = out0 /\ outoff = outoff0 /\ sp_0 = state0 /\
    W64.to_uint outoff0 + 136 <= BArray8192.size ==>
    (forall j, 0 <= j < 136 =>
      BArray8192.get8 res.`1 (W64.to_uint outoff0 + j) = BArray200.get8 res.`2 j) /\
    SHAKEBlockSpec.rate_block_frame out0 res.`1 (W64.to_uint outoff0) 136 /\
    word_state_of_barray res.`2 = keccak_f1600 (word_state_of_barray state0)].
proof.
  conseq (squeeze256_from_state_contract out0 outoff0 state0
    (fun state => word_state_of_barray state = keccak_f1600 (word_state_of_barray state0))
    (keccakf1600_word_correct state0)) => />.
  smt(SHAKEBlockSpec.rate_block_bytes).
qed.

lemma squeeze256_word_total
    (out0 : BArray8192.t) (outoff0 : W64.t) (state0 : BArray200.t) :
  phoare [Signer.__sample_full_squeeze256 :
    outp = out0 /\ outoff = outoff0 /\ sp_0 = state0 /\
    0 <= W64.to_uint outoff0 <= 8056 ==>
    (forall j, 0 <= j < 136 =>
      BArray8192.get8 res.`1 (W64.to_uint outoff0 + j) = BArray200.get8 res.`2 j) /\
    SHAKEBlockSpec.rate_block_frame out0 res.`1 (W64.to_uint outoff0) 136 /\
    word_state_of_barray res.`2 = keccak_f1600 (word_state_of_barray state0)] = 1%r.
proof.
  by conseq squeeze256_ll (squeeze256_word_correct out0 outoff0 state0) => /> /#.
qed.

end SHAKEBlockCorrectness.
