...
```sh
make -C haetae-1.2.0-easycrypt verify-new        # NTT 외 로컬 명세와 증명 106개
...
승인 후 조건부분포의 정규화와 오차도 증명했다. 이상적 조건부분포의
별도 Gaussian 확률 질량식과의 식별, 부호 및 반복 표본 합성이 남아 있다. 전체 거부 루프의 종료·분포와 전체 서명 정확성·보안도 남아 있다.
