...
Week~8--15의 주 실행 경로는 실제 서명 코덱의 HBZ rANS 접미부였다. 계수/기호 잎, 표,
...
두 row consequence를 77번째 대상으로 컴파일했다. 필요한 convolution 층이
동결 표면에는 없어 \identifier{STOP-KG-NTT}로 고정했고, 뒤이어 Sign/Verify
제한 결과까지 82개 \identifier{PAPER-FROZEN} 경계에 포함했다. post-freeze에는
순수 NTT/Rq 표현과 actual-(a)/paper-(qj) 보조 층이 닫혔지만 synthetic
security object의 의미 불일치 때문에 최신 관문(gate)은
...
paper-freeze 당시 첫 누락 잎은 실제 Montgomery 인자를 포함한 odd-root 직교성/full-NTT
spectral-action, 즉 점별 곱의 inverse NTT와 \identifier{Rq.\&*} negacyclic
convolution을 동일시하는 정리였다. 그 뒤에는 \identifier{Rq.poly} 배열을
\identifier{HAETAE_Algebra.poly_mul} 정수 목록으로 옮기는 별도 adapter가
필요했다. 따라서 동결 주장 행렬에서 faithful KG-1, KG-3, 전체 KG-4와
\[
  A s=qj\pmod{2q}
\]
는 증명되지 않았다. 동결 판정 \identifier{STOP-KG-NTT}는 KeyGen을 KG-2 및
finalization 경계에서 고정했다. 이후 실제 세 Sign helper의 직접 호출과
accepted-branch control은 컴파일됐지만 다음 정리가 없다.
\begin{center}
  \small\identifier{sf_challenge_mode2_highbits_lsb_sampleinball_correct}
\end{center}
따라서 \identifier{STOP-SIGN-CHAL-MODE2}로 동결되었다. S-1--S-7 headline 정리는
미증명이다. 이어진 MINCORE-VERIFY는 helper-local machine-word/control 결과를
보존했다.

paper-freeze 이후 \identifier{rq_mul_coeff_foldr_to_bigi},
\identifier{full_ntt_montgomery_}\allowbreak\identifier{spectral_action},
일반 row-product와
Rq--HAETAE 곱 표현 adapter는 별도 보조 트리에서 닫혔다. actual mode-2
\identifier{avec}도 SHAKE128/rejection 의미를 거쳐 논문 \(a\)로 연결되고,
논문 \(qj\) 항은 snapshot 식에 독립적으로 더해졌다. 그러나 기존 보안 모델의
synthetic \identifier{haetae_mode23_qj_vector}가 이 객체들과 다름이 증명되어
최신 판정은 \identifier{STOP-KG-AVEC-QJ}다. 따라서 KG-1--KG-4 및 위 논문 식은
여전히 승격되지 않는다. Verify에서는 순수 NTT 기반 대신 실제 2-by-4
\identifier{verify_matrix_ntt_acc_mode2_cols4_correct}와
\identifier{verify_crt_freeze_mode2_word_exact}가 combined theorem의 첫 절차
blocker다.
