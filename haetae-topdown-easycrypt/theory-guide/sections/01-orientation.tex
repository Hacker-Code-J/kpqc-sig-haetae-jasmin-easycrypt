...
82개 EasyCrypt 대상; Verify 수정 전 보존 전체 완료 기준선은 78개 \\
최신 판정 & \identifier{STOP-VERIFY-MATRIX-CRT};
\identifier{OBL-MINCORE-KEYGEN}은 실제 matrix/finalizer 스냅샷 경계에서
\statuspartial 이고 Sign은 actual-call control에서 \statuspartial; Verify의 첫
재개 leaf는 \identifier{rq_mul_coeff_foldr_to_bigi}, 이어 2-by-4 NTT/CRT leaf \\
...
\begin{boundary}[82대상 매니페스트와 완료 증거]
...
\sourcepath{logs/verify-all-before-week16-verify.log}에 보존된다. 실행 중 덮어쓰는
\sourcepath{logs/verify-all-week16-verify-matrix-crt.log}는 마지막 줄
\texttt{RESULT PASS authored-targets=82 cache=-no-eco}와 SHA-256
\texttt{4cd64e5a656be82710bca1410c4d19403a3c661d6b91b0319a0ea8f7c91646da}를
가지는 보존된 현재 전체 완료 증거다. Hoare 정리의 \statusproved 는 여전히 실제 절차의
종료나 확률 1 실행을 뜻하지 않는다.
\end{boundary}
