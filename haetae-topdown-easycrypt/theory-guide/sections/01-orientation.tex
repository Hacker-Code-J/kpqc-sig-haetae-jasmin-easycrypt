...
78개 EasyCrypt 대상; Sign 수정 전 보존 전체 완료 기준선은 77개 \\
최신 판정 & \identifier{STOP-SIGN-CHAL-MODE2};
\identifier{OBL-MINCORE-KEYGEN}은 실제 matrix/finalizer 스냅샷 경계에서
\statuspartial 이고 Sign은 actual-call control에서 \statuspartial; 활성 전선은 Verify \\
...
\begin{boundary}[78대상 매니페스트와 완료 증거]
77번째 대상 \sourcepath{Mode2KeygenNttMulBridge.ec} 뒤에 78번째
\sourcepath{Mode2SignAcceptedCore.ec}가 actual Sign control boundary를 fresh
compile한다. Sign 수정 전 77대상 전체 완료 실행은
\sourcepath{logs/verify-all-before-week16-sign.log}에 보존된다. 실행 중 덮어쓰는
\sourcepath{logs/verify-all-summary.txt}는 마지막 줄에
\texttt{RESULT PASS authored-targets=78 cache=-no-eco}가 있을 때만 현재
78대상 전체 완료 증거다. Hoare 정리의 \statusproved 는 여전히 실제 절차의
