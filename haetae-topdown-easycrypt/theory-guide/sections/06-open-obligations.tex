...
\subsection{KeyGen 관문: 동결 STOP-KG-NTT에서 최신 STOP-KG-AVEC-QJ까지}
...
\begin{boundary}[paper-freeze의 \claim{STOP-KG-NTT}]
...
\identifier{Rq.poly}--security-list adapter가 필요했다. 둘 다 82대상 동결
경계에는 없으므로 그 주장 행렬은 faithful KG-1, KG-3, 전체 KG-4와
\(As=qj\pmod{2q}\)를 구성하지 않는다.
\end{boundary}

post-freeze에는 \identifier{rq_mul_coeff_foldr_to_bigi}, full-NTT spectral
action, 일반/mode-2 row product와 Rq--HAETAE 계수 표현 adapter가 닫혔다. 이어
actual mode-2 sampler의 nonce 515/516, SHAKE128 framing, little-endian parsing과
rejection 의미가 paper \(a\)로 운반되고, 별도 paper \(qj\) correction이 기존
snapshot 등식과 합성됐다.

\begin{boundary}[최신 \claim{STOP-KG-AVEC-QJ}]
기존 보안 모델의 \identifier{public_rounding_vector_seed}는 tag 401 기반
synthetic generator다. 이는 actual sampler와도 paper \(qj\)와도 같지 않으며,
\identifier{security_named_qj_is_not_paper_qj}가 row~0 coefficient~0에서
\(401\ne64513\)을 증명한다. all-zero raw seed의 실제 nonce-515 첫 계수도
44985이므로 이 동일성을 가정할 수 없다. 보안 모델을 actual ExpandVecA 의미로
교체ㆍ정제하는 \claim{OBL-KG-SECURITY-EXPANDVECA-SEMANTICS}가 KG 승격보다 먼저다.
\end{boundary}

따라서 동결 \claim{OBL-MINCORE-KEYGEN} 상태는 KG-2/finalization의
\statuspartial 로 유지된다. 최신 보조 전선도 full KG-1--KG-4를 승격하지 않는다.
MINCORE Sign의 동결 판정은
\identifier{STOP-SIGN-CHAL-MODE2}다. MINCORE Verify의 동결 판정은
\identifier{STOP-VERIFY-MATRIX-CRT}이며, 제한 합성은 \statusspecified 다.
Verify의 순수 NTT 기반은 post-freeze에 닫혔지만 실제
\identifier{verify_matrix_ntt_acc_}\allowbreak
\identifier{mode2_cols4_correct}와
\identifier{verify_crt_freeze_mode2_word_exact} 절차 leaf가 남는다.
...
실제 snapshot/finalizer와 마지막 \identifier{output_row} rewrite까지인 동결
82대상 주장. post-freeze NTT/Rq 보조 정리는 이 역사적 stop을 전진시켰지만
동결 claim 자체와 full KG 식은 승격하지 않음 \\
\claim{OBL-KG-}\newline
\claim{SECURITY-}\newline
\claim{EXPANDVECA-}\newline
\claim{SEMANTICS} & 기능정확성;\newline
\statusblocked;\newline\identifier{STOP-KG-}\newline
\identifier{AVEC-QJ} & synthetic
\identifier{public_rounding_vector_seed}를 actual SHAKE128/LE16/rejection 및
nonce 515/516 의미로 교체ㆍ정제하고 paper \(qj\)를 별도 correction으로 유지한
actual-array 표현 정리가 필요 \\
\claim{OBL-MINCORE-SIGN} & 기능정확성;\newline\statuspartial;\newline
\identifier{STOP-SIGN-}\allowbreak\identifier{CHAL-MODE2} & 실제 round-challenge, z-check, hint helper
직접 호출과 accepted-branch control만 컴파일; full challenge semantics와
S-1--S-7 headline은 미증명 \\
\claim{OBL-MINCORE-VERIFY} & 기능정확성;\newline\statuspartial;\newline
\identifier{STOP-VERIFY-}\allowbreak\identifier{MATRIX-CRT} & actual helper 순서, V-1/V-2/V-5/V-6
machine-word 식, W64 norm gate와 tail/mismatch word expression은 컴파일;
post-freeze 순수 NTT 기반은 증명됐으나 실제 2-by-4 matrix accumulation,
CRT/freeze 의미 bridge와 challenge equality는 미증명 \\
...
\begin{enumerate}
  ...
  \item Week~14 production 래퍼와 Week~15 고정 all-6 성공 정리를 유지하고
        ...
        별도 의무로 남긴다.
  \item 동결 주장 행렬의 \identifier{STOP-KG-NTT}는 역사적 경계로 유지한다.
        post-freeze에 닫힌 NTT/Rq--HAETAE 정리를 재사용하되 이를 동결 PDF의
        claim으로 소급 기록하지 않는다.
  \item 최신 \identifier{STOP-KG-AVEC-QJ}를 먼저 해결한다. synthetic
        \identifier{public_rounding_vector_seed}를 actual ExpandVecA 의미로
        교체ㆍ정제하고 paper \(qj\) correction과 분리한 뒤
        \identifier{OBL-KG-SECURITY-EXPANDVECA-SEMANTICS}를 증명한다.
  \item 보안 \(a\), actual array와 Rq/HAETAE row product를 연결한 뒤 mode-2
        폭 4/mod-\(q\) 표현을 논문 augmented 폭 6/mod-\(2q\) matrixㆍsecretㆍerror
        packaging으로 운반한다. 그 전에는 faithful KG-1/KG-3/전체 KG-4를
        주장하지 않는다.
  \item \identifier{STOP-SIGN-CHAL-MODE2}를 유지한다. full
        highbits/LSB/\(\mu\)-to-SampleInBall leaf, 실제 Sign row-product 적용과
        response/hint 표현 의무 없이 S-1--S-7을 주장하지 않는다.
  \item Verify에서는 이미 닫힌 순수 NTT 기반을 실제 2-by-4 accumulation에
        적용하는 \identifier{verify_matrix_ntt_acc_}\allowbreak
        \identifier{mode2_cols4_correct}와
        \identifier{verify_crt_freeze_mode2_word_exact}를 닫아 combined matrix
