...
최신 판정은 \identifier{STOP-KG-NTT}다. \claim{OBL-MINCORE-KEYGEN}은
KG-2/finalization에서 \statuspartial 로 동결되었다. MINCORE Sign/Verify/제한
합성은 \statusspecified 이며 \identifier{GO-KG}는 허용되지 않는다. 현재 단일
목표는 actual accepted Sign core다.
...
\claim{OBL-MINCORE-SIGN} & 기능정확성;\newline\statusspecified & 실제
round-challenge, z-check, hint helper의 accepted-core 식이 현재 단일 목표; 아직
headline 정리 미컴파일 \\
\claim{OBL-MINCORE-VERIFY},\newline
\claim{OBL-MINCORE-COMPOSITION} &
기능정확성;\newline\statusspecified & canonical decoded Verify core 술어와
decoded-object 경계의 제한 합성; 아직 개별 정리 미컴파일 \\
...
\begin{enumerate}
  ...
  \item 현재 단일 목표인
        \identifier{actual_sign_accepted_core_equations_mode2}를 실제
        round-challenge, z-check, hint helper 합성으로 닫는다.
  \item 그 다음 canonical decoded Verify core 술어와 decoded-object 제한 합성을
        각각 닫는다.
