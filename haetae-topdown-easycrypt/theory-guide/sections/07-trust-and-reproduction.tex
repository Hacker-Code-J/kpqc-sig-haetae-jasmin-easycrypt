...
\begin{enumerate}
  ...
  \item Week~9 rANS 정리 표면에 더해 Week~10 배열/목록, 단어 단계, 내부 진행,
        ...
        직접 Sign core control과 \identifier{STOP-SIGN-CHAL-MODE2} 표면 검사;
  \item 현재 매니페스트된 78개 작성 EasyCrypt 대상의 \identifier{-no-eco} 새
        컴파일;
...
\sourcepath{logs/verify-all-before-week16-sign.log}에 보존된다.
...
\sourcepath{logs/verify-all-summary.txt}는 마지막 줄이
\texttt{RESULT PASS authored-targets=78 cache=-no-eco}일 때만 현재 78대상
완료 실행의 증거다. 그 줄이 없는 중간 또는 중단 실행은 보존된 77대상
Sign 수정 전 기준선을 갱신하지 않는다.
\end{boundary}
...
78대상 매니페스트다. Sign 수정 전 77대상 완료 로그가 보존되어 있다. 전체
78대상 완료는 위 종료 표식까지 확인한 실행에만 주장한다.
