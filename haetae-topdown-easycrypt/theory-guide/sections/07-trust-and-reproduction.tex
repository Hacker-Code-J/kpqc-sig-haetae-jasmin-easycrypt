...
\begin{enumerate}
  ...
  \item Week~9 rANS 정리 표면에 더해 Week~10 배열/목록, 단어 단계, 내부 진행,
        ...
        Week~16 실제 M23 matrix/finalizer 스냅샷, 마지막 NTT 표현 rewrite와
        \identifier{STOP-KG-NTT} 표면 검사;
  \item 현재 매니페스트된 77개 작성 EasyCrypt 대상의 \identifier{-no-eco} 새
        컴파일;
...
\texttt{cache=-no-eco}다. \sourcepath{WEEK16_KG_NTT_MUL_REPORT.md}는 77번째
대상의 개별 fresh compile, 새 표현 경계와 중단 판정을 설명한다. 현재 체크아웃의
개별 재실행도 성공했다. 다만 최종 77대상 전체 완료 로그는 아직 없다.

\begin{boundary}[76대상 수정 전 기준선과 77대상 continuation]
...
실행 중 생성되는 \sourcepath{logs/verify-all-}\allowbreak
\sourcepath{summary.txt}는 마지막 줄이
\texttt{RESULT PASS authored-targets=77 cache=-no-eco}일 때만 현재 77대상
완료 실행의 증거다. 그 줄이 없는 중간 또는
중단 실행은 보존된 완료 기준선을 갱신하지 않는다. 따라서 이 파일이 다른
실행에 의해 덮어써지는 동안에는 위 76대상 보존 로그를 사용한다.
\end{boundary}
...
77대상 매니페스트다. 마지막 보존 전체 완료는 76대상이고 77번째 대상은 개별
컴파일됐다. 전체 77대상 완료는 위 종료 표식까지 확인한 실행에만 주장한다.
덮어쓰기 중인 summary의 중간 PASS 줄은 완료 증거가 아니다. 이 안내서의 별도
빌드도 함께 확인한다.
\end{takeaway}
