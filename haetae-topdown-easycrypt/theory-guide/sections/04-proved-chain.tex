...
67대상 \identifier{-no-eco} 전체 검증과 독립 정리 표면 검사가 통과했고
Week~13 당시 판정은 \identifier{GO-CORE}다. Week~14의 단일 구현 간선은 이미 닫힌
...
\subsection{16단계: Week~14 production 전체 HBZ 래퍼 합성}

Week~14는 Week~13이 남긴 production 경계를 다섯 작성 이론으로 닫았다.
\sourcepath{Mode2HbzInternal}\allowbreak\sourcepath{Boundaries.ec}는
full-wrapper 추출물과 기존 focused
추출물의 prepare, rANS encode/decode, apply 절차를 정확 동치로 연결한다. 다음
두 어댑터는 계수와 기호 경계를 잇는다.

\begin{itemize}
  \item \identifier{prepared_hbz_implies_mode2_symbol_stream}: 정준 HBZ의 실제
        prepare 결과를 Week~13 핵심 역함수의 13기호 stream 전제로 운반한다.
  \item \identifier{decoded_prefix_preserves_prepared_hbz}: 실제 디코더가 복원한
        기호 접두부를 apply 정리의 prepared-HBZ 전제로 되돌린다.
\end{itemize}

\sourcepath{Mode2HbzFullEncodeTrace.ec}와
\sourcepath{Mode2HbzFullDecodeInverse.ec}는 실제 full encoder/decoder를 각각
직접 대상으로 삼는다. 전자는 실제 prepare-local 기호와 정확한 encoded trace,
크기 및 suffix frame을, 후자는 그 trace에서 \(bad=0\), 원래 1024계수와
coefficient tail frame을 결론낸다. 두 결과는
\sourcepath{Mode2HbzFullActualInverse.ec}의 직접 결합에 합성된다.

\begin{verified}[production full-HBZ 래퍼 역함수]
\label{thm:production-hbz-wrapper-inverse}
\sourcepath{Mode2HbzSignatureBoundaryLift.ec}의 경계 lift는 다음 두 실행의
결과 튜플과 \identifier{decoder_ran} 플래그가 정확히 같음을 증명한다.
\begin{itemize}[leftmargin=2em]
  \item production: \identifier{SignaturePackUnpackHbzFullHarness.run}
  \item focused: \identifier{HbzFullActualHarness.run}
\end{itemize}
정확한 동치 정리명은
\identifier{signature_pack_unpack_hbz_full_actual_exact}이고, 그 Hoare
따름정리 \identifier{signature_pack_unpack_hbz_full_inverse_mode2}는 정준
HBZ와 초기 인자 바인딩만을 전제로 다음을 보인다.
\[
\begin{array}{ll}
size=0 &: \neg\mathsf{decoder\_ran}\text{ 및 초기 버퍼 보존},\\
size\ne0 &: 4\le\operatorname{uint}(size)\le1024,
\ \mathsf{decoder\_ran},\ bad=0,\\
& decoded[0..1024)=hbz_0[0..1024)\text{ 및 두 꼬리 프레임}.
\end{array}
\]
성공 분기는 actual prepare의 \identifier{prepared_symbols}와 정확한
\identifier{mode2_trace_bytes} 증인도 제공한다. 성공이나 디코더 출력을
사전조건으로 요구하지 않으므로 순환 합성이 아니다.
\end{verified}

Week~14 완료 상태는 다음과 같다.
...
의무 & 상태 \\
\midrule
full/focused 내부 절차 동치와 계수--기호 어댑터 & \statusproved \\
실제 full-HBZ encoder/decoder 직접 정제 & \statusproved \\
\claim{OBL-SIG-HBZ-ENCODE-DECODE} & \statusproved\ (성공 조건부 부분정확성) \\
\claim{OBL-RANS-ACTUAL-SUCCESS-WITNESS} & \statuspartial \\
인코더/디코더 종료 & \statuspartial \\
\claim{OBL-SIG-H-ENCODE-DECODE} & \statusspecified \\
\bottomrule
\end{tabularx}
\end{center}

다섯 새 full-HBZ 이론을 포함한 72대상 \identifier{-no-eco} 전체 검증이
통과했고 Week~14 당시 판정은 \identifier{GO-HBZ}다. 다음 단일 구현 의무는 all-zero
HBZ 계수---prepare 뒤 모두 6인 기호열---가 실제 nonzero-size 성공 분기에
도달함을 EasyCrypt 안에서 보이는 것이다. 이 증인 전에는 무조건 왕복, (h)
코덱, 전체 1474바이트 코덱이나 인코딩 무손실로 넓히지 않는다.

\subsection{17단계: Week~15 고정 all-6 실제 성공 사후조건}

Week~15는 두 작성 이론과 기존 인코더 폐쇄 정리의 최소 강화를 사용했다.
\sourcepath{Mode2RansAllSix}\allowbreak\sourcepath{Budget.ec}는 다음 연결을
제공한다.

\begin{itemize}
  \item bytewise \identifier{zero_hbz}의 실제 32비트 읽기에서 정준 HBZ와
        실제 prepare의 all-6 기호 접두부를 구성한다.
  \item 기호 6은 상태 \(x<2^{31}\)에서 정규화 바이트를 최대 하나만 내보내며,
        첫 네 단계는 0바이트임을 계산한다.
  \item 따라서 1024기호의 정규화 추적은 최대 1020바이트이고 마지막 상태
        4바이트를 합친 전체 추적이 고정 1024바이트 작업공간에 들어간다.
\end{itemize}

기존 \sourcepath{Mode2RansEncoderActual}\allowbreak
\sourcepath{TraceClosure.ec}를 강화했다.
\identifier{actual_rans_encode_}\allowbreak\identifier{failure_trace_cause}는
실제 실패가
1020바이트보다 긴 정규화 추적을 요구함을 기존 반복 불변식에서 운반한다.
순수 상계와의 모순으로 다음 실제 정리가 닫힌다.

\begin{verified}[고정 all-6 production HBZ 성공]
\label{thm:all-six-production-success}
\sourcepath{Mode2RansActual}\allowbreak\sourcepath{SuccessWitness.ec}의
\identifier{actual_rans_encode_all_six_success}는 실제
\identifier{RansEncodeTarget.M._rans_encode}를 직접 대상으로 하며, 성공이나
버퍼 적합성을 사전조건으로 받지 않는다. 종료한 실행에서
\[
 bad=0,\qquad 0\le off\le1020,\qquad
 4\le1024-off\le1024
\]
와 정확한 all-6 trace 및 접두부 프레임을 준다.
\identifier{actual_encode_hb_z1_full_zero_success}와
\identifier{signature_pack_hbz_zero_success_mode2}가 이 결론을 실제 full
encoder와 production packer로 올린다. 마지막으로
\identifier{signature_pack_unpack_hbz_zero_success_mode2}는 고정
\identifier{zero_hbz} 입력의 production 결합에서 nonzero size, 디코더 실행,
\(bad=0\), 1024개 zero 계수 복원과 두 꼬리 프레임을 결론낸다.
\end{verified}

Week~15 완료 상태는 다음과 같다.
...
의무 & 상태 \\
\midrule
zero-HBZ prepare→all-6 및 1020바이트 예산 & \statusproved \\
\claim{OBL-RANS-ACTUAL-SUCCESS-WITNESS} &
  \statusproved\ (고정 입력 Hoare 부분정확성) \\
고정 zero-HBZ production 왕복 & \statusproved\ (Hoare 부분정확성) \\
고정 입력 및 일반 rANS 종료/losslessness & \statuspartial \\
\bottomrule
\end{tabularx}
\end{center}

두 새 이론을 포함한 74대상 전체 검증이 통과했고 Week~15 판정은
\identifier{GO-WITNESS}다. 이는 \identifier{GO-WITNESS-LOSSLESS}가 아니다.
실행 존재, 종료, 확률 1 성공과 모든 정준 HBZ 입력의 성공은 따라오지 않는다.
이후의 1주 최소 논문 범위가 원래 (h) 코덱 권고를 덮어써 (h)는 연기되었다.

\subsection{18단계: Week~16 MINCORE KeyGen 스냅샷}

Week~16의 첫 과제는 전체 KeyGen이나 API가 아니라 실제 두 보조절차의 투명한
합성이다. \sourcepath{Mode2KeygenSnapshotAlgebra.ec}는 정준 잔여 \(r\)에 대해
\[
  r=2\operatorname{high}(r)+\operatorname{low}(r),\qquad
  -1\le\operatorname{low}(r)\le1
\]
과 원시 합/잔여의 mod-\(2q\) 합동을 순수 정수 정리로 증명한다.
\sourcepath{Mode2KeygenCoreEquation.ec}의
\identifier{ActualM23MatrixFinalizeSnapshot.run}은 실제
\identifier{_kp_m23_matrix}와 \identifier{_keypair_finalize_m23}를 순서대로
직접 호출하고 중간 \(bp\)를 \identifier{pre_bp}로 보존한다.

\begin{verified}[실제 matrix/finalizer 의미 스냅샷]
\label{thm:actual-keygen-snapshot}
\identifier{actual_m23_matrix_finalize_semantic_snapshot}는 입력/행렬 범위와
초기 배열 바인딩만을 전제로 다음을 함께 결론낸다.
\begin{itemize}
  \item 실제 matrix helper의 \identifier{mode2_output_repr_bound16} 및
        \identifier{mode2_ntt_repr_bound24};
  \item actual finalizer의 정확한 배열/HAETAE 의미와
        \(r=2b_1+b_0,\ -1\le b_0\le1,\ s'_2=s_2-b_0\);
  \item 각 활성 계수의
        \(2(a-2b_1)+2\,pre\_bp+2s'_2\equiv0\pmod{2q}\);
  \item 초기→스냅샷→최종, NTT scratch와 \(s_2\) 꼬리 프레임.
\end{itemize}
최종 출력이나 원하는 KG 술어를 사전조건으로 넣지 않는다.
\end{verified}

그러나 \identifier{pre_bp}는
\identifier{KeygenM23ArithmeticSpec.output_row}가 표현하는 full-NTT/점별
곱/inverse-NTT 스냅샷일 뿐이다. 이를 논문의
\(A_{\mathrm{gen}}s_{\mathrm{gen}}\) 다항식 곱과 동일시하는 정리가 없었으므로
당시 상태는 다음과 같았다.
...
의무 & 상태 \\
\midrule
actual matrix/finalizer 스냅샷 의미 & \statusproved \\
KG-2형 실제 계수 low/high 분해 & \statusproved \\
KG-4의 조정된 \(s_2\) 성분 / 전체 KG-4 & \statusproved\ /\ \statuspartial \\
faithful KG-1, KG-3, 논문 \(As=qj\bmod2q\) & \statusblocked \\
\claim{OBL-MINCORE-SIGN}, \claim{OBL-MINCORE-VERIFY},
\claim{OBL-MINCORE-COMPOSITION} & \statuspartial\ /\ \statuspartial\ /\ \statusspecified \\
\claim{OBL-SIG-H-ENCODE-DECODE} & \statusdeferred \\
\bottomrule
\end{tabularx}
\end{center}

두 KeyGen snapshot 이론을 더한 76대상 수정 전 기준선은 전체 검증을
통과했다. 첫 과제의 역사적 판정은 \identifier{CONTINUE-KG}와
\identifier{BLOCKED-KG-NTT-MUL}이었고, 아래 continuation 감사가 이를 최종
중단 판정으로 갱신했다.

\subsection{19단계: Week~16 KG-NTT-MUL 감사와 중단 판정}

후속 과제는 원하는 보안 모델 등식을 전제로 넣지 않고 기존 실제 NTT chain의
마지막 표현 간선을 소스에 명시했다. 77번째 매니페스트 대상
\sourcepath{Mode2KeygenNttMulBridge.ec}의
\identifier{output_row_from_mode2_ntt_words}는 변환된 비밀키 표현 아래
\[
 \mathsf{output\_row}
 =\operatorname{array256\_mont}
   (\operatorname{full\_invntt}(\operatorname{pointwise\_row\_words}(\cdots)))
\]
를 진술하고, \identifier{output_row_repr_from_mode2_ntt_words}는 그 등식을
기존 \identifier{wide_slice_repr_bound} 계약으로 운반한다.
\identifier{actual_m23_matrix_snapshot_rows_explicit}는 기존 actual two-call
snapshot theorem의 consequence로 이 표현을 두 active row의 사후조건에 직접
적용한다. 세 정리는
\identifier{Rq.\&*}, \identifier{HAETAE_Algebra.poly_mul},
\(A_{\mathrm{gen}}s_{\mathrm{gen}}\) 또는 KG 술어를 언급하거나 전제하지 않는다.
세 정리와 direct harness import를 포함한 파일은 개별
\identifier{-no-eco} fresh compile을 통과했다.

\begin{boundary}[paper-freeze 시점 KG-NTT-MUL의 두 누락 잎]
첫째, 실제 Montgomery 인자를 포함해
\[
 \operatorname{mont}
  (\operatorname{full\_invntt}(\widehat a\odot\operatorname{full\_ntt}(p)))
 =\operatorname{full\_invntt}(\widehat a)\mathbin{\star_{\!Rq}}p
\]
를 보이는 odd-root 직교성/full-NTT convolution 정리가 없었다. 둘째, native
\identifier{Rq.poly} 배열의 곱을 보안 모델의 정수-list
\identifier{poly_mul}/\identifier{matrix_vec_mul}로 전사하는 adapter도 없었다.
legacy \identifier{Rq.ntt}는 실제 full transform과 다르므로 이 공백을 메울 수
없다. 따라서 snapshot의 \identifier{pre_bp}를
\(A_{\mathrm{gen}}s_{\mathrm{gen}}\)으로 다시 부르는 것은 순환이다.
\end{boundary}

동결 판정은 \identifier{STOP-KG-NTT}다. KG-2/finalization은
\statusproved, KG-4의 조정된 \(s_2\) 성분만 \statusproved, faithful KG-1/KG-3와
전체 KG-4 및 논문 식은 증명되지 않았다. KeyGen 전선은 이 경계에서 동결된다.
그 뒤 78번째 대상 \identifier{Mode2SignAcceptedCore}가 실제 세 helper의 직접
호출과 accepted-branch control을 컴파일했다. 그러나 full challenge semantics
leaf가 없어 판정은 \identifier{STOP-SIGN-CHAL-MODE2}이며 S-1--S-7은
미증명이다. 이어진 네 Verify 대상은 actual 다섯 helper의 순차 control,
V-1/V-2/V-5/V-6의 exact machine-word 식, W64 norm gate와 tail/mismatch word
expression을 컴파일했다. matrix CRT 의미 bridge가 없으므로 판정은
\identifier{STOP-VERIFY-MATRIX-CRT}이며 동결 시점 첫 재개 leaf는
\identifier{rq_mul_coeff_foldr_to_bigi}, 이어 2-by-4 NTT/CRT 절차 leaf였다.

\subsection{20단계: paper-freeze와 post-freeze KeyGen 의미 정렬}
\label{subsec:post-freeze-keygen}

커밋 \identifier{4bde1207e2a290952d491d5b41c4bd773081e3c4}는 새 정리를
추가하지 않고 82개 작성 대상, 주장 원장, 정리 그래프, 재현 절차와 논문 PDF를
같은 \identifier{PAPER-FROZEN} 경계로 맞췄다. 동결 PDF의 SHA-256은
\hashsplit{be935948028829556951863b44ceb2e6}{c5b2037820991b3a43a4b461b036e53d}다.
이후 작업은 이 논문 주장 행렬을 소급 변경하지 않고, 외부 NTT 트리와 기존
77번째 KeyGen 대상, 그리고 \sourcepath{post-freeze/} 보조 대상에서 진행됐다.

첫 보조 층은 paper-freeze 때 막혔던 순수 NTT와 표현 간선을 닫는다.
\sourcepath{haetae-ntt-verify/easycrypt/}\allowbreak
\sourcepath{NTTFullSpectralAction.ec}는
\identifier{rq_mul_coeff_foldr_to_bigi},
\identifier{full_ntt_montgomery_spectral_action} 및 일반
\identifier{full_ntt_montgomery_row_product}를 증명한다. 실제 full transform과
다른 legacy \identifier{Rq.ntt}를 사용하지 않는다.
\sourcepath{NTTMode2Row}\allowbreak\sourcepath{Specializations.ec}는 이 결과를 KeyGen의 2-by-3과
Verify의 2-by-4 수학적 row product에 특수화한다.

\sourcepath{RqHAETAEBridge.ec}는 계수별 표현 관계
\identifier{rq_poly_repr}를 도입한다. 이 관계의 전제는 HAETAE 다항식의
well-formedness와 활성 계수 등식뿐이며 원하는 convolution이나 KG 식을 포함하지
않는다. 다음 정리들이 fresh compile됐다.

\begin{itemize}
  \item \identifier{rq_poly_add_repr}: \identifier{Rq.\&+}와
        \identifier{HAETAE_Algebra.poly_add}의 표현 보존;
  \item \identifier{rq_poly_mul_repr}: checked negacyclic
        \identifier{Rq.\&*}와 \identifier{HAETAE_Algebra.poly_mul}의 표현 보존;
  \item \identifier{rq_poly_dot3_repr}: 세 곱의 합을
        \identifier{HAETAE_Algebra.poly_dot}으로 운반;
  \item \identifier{keygen_mode2_row_product}와
        \identifier{mode2_row_product_haetae_dot}: 실제 KeyGen NTT 출력에서
        native Rq row product와 HAETAE 3항 dot으로 연결.
\end{itemize}

따라서 paper-freeze 당시의 odd-root/full-NTT 및 Rq--HAETAE 두 순수 잎은 더
이상 전역 미해결 의무가 아니다. 다만 이 사실만으로 actual matrix words,
security-side \(A\), secret/error packaging과 논문 mod-\(2q\) 식이 자동으로
같아지지는 않는다.

두 번째 보조 층은 종료한 실제 mode-2 \identifier{avec} 확장의 결정론적
의미를 논문 객체와 맞춘다.
\sourcepath{post-freeze/KgActualAvecQjSemantics.ec}는 다음을 증명한다.

\begin{itemize}
  \item 두 row nonce는 각각 515와 516이고, SHAKE128 입력은 32바이트 seed 뒤에
        nonce 두 바이트를 붙인 형태다;
  \item domain byte 31, 마지막 padding byte 128, 168바이트 squeeze block과
        최초 네 block의 672바이트 크기가 실제 framing과 일치한다;
  \item 인접 두 바이트를 little-endian 16-bit 후보로 읽고 \(64513\) 미만만
        받아들이며, 수락한 계수는 추가 modular reduction 없이 저장된다;
  \item 이 과정을 통과한 두 256계수 row는 논문 수준 mode-2 벡터 \(a\)다.
\end{itemize}

논문의 \(j=(1,0)^T\)에서 첫 \(1\)은 상수다항식의 항등원이므로 \(qj\)는 row~0의
coefficient~0에만 \(q=64513\)을 더하고 나머지 계수는 0이다.
이 경우는 \identifier{paper_qj_row0_coeff0},
\identifier{paper_qj_row0_other}, \identifier{paper_qj_remaining_row}가 분리한다.

\sourcepath{post-freeze/}\allowbreak
\sourcepath{KgActualAvecQjComposition.ec}의 두 보조 정리
\begin{center}
\small
\identifier{mode2_sampler_facts_actual_paper_a},\qquad
\identifier{actual_snapshot_zero_adds_paper_qj}
\end{center}
는 actual sampler 사실과 기존 snapshot mod-\(2q\) 식에 이 두 paper 객체를
합성한다. 최상위 Hoare 정리
\begin{center}
\small
\identifier{checked_mode2_parent_m23_finalize_actual_avec_qj}
\end{center}
는 checked sampler, 실제 matrix helper와 finalizer의 종료한 실행에 대해 두
결과를 함께 내보낸다. sampler 종료나 분포는 결론이 아니다.

\begin{boundary}[\identifier{STOP-KG-AVEC-QJ}]
기존 보안 모델의
\identifier{haetae_mode23_qj_vector}는 실제 ExpandVecA나 논문 \(qj\)가 아니라
\identifier{public_key_seed_polyveck}에 tag 401을 넣은 synthetic 산술
generator다. 컴파일된 정리는 모든 seed에서 그 row~0/1의 coefficient~0을 각각
401과 403으로 계산하지만 paper \(qj[0][0]=64513\)이다.
\identifier{security_named_qj_is_not_paper_qj}가 이 부등식을 직접 증명한다.
독립 FIPS-202 all-zero-seed trace에서도 실제 nonce-515 row~0 첫 계수는 44985로,
synthetic 값 401과 다르다. 원하는 동일성을 전제로 넣으면 구체적으로
\(44985=401\)을 요구하므로 sound하지 않다.
\end{boundary}

따라서 최신 최소 수리 의무는
\identifier{OBL-KG-SECURITY-EXPANDVECA-SEMANTICS}다. 보안 쪽
\identifier{public_rounding_vector_seed}를 실제 SHAKE128, little-endian parsing,
\(<64513\) rejection과 nonce 515/516 의미로 교체하거나 정제하고, paper \(qj\)
correction은 별도 mod-\(2q\) 객체로 유지한 뒤 actual-array 표현을 증명해야 한다.
그 다음에도 현재 HAETAE 대수의 mode-2 폭 4/mod-\(q\) 표현과 논문 augmented
matrix의 폭 \(1+3+2=6\)/mod-\(2q\) packaging을 연결해야 한다. 그러므로 이
단계는 KG-1--KG-4, \(As=qj\pmod{2q}\), sampler distribution, termination 또는
\(\delta_{\mathrm{KeyGen}}=0\)을 주장하지 않는다.
