...
\(A_{\mathrm{gen}}s_{\mathrm{gen}}\) 다항식 곱과 동일시하는 정리가 없으므로
현재 상태는 다음과 같다.
...
\begin{boundary}[KG-NTT-MUL에서 확인된 두 누락 잎]
...
\]
를 보이는 odd-root 직교성/full-NTT convolution 정리가 없다. 둘째, native
\identifier{Rq.poly} 배열의 곱을 보안 모델의 정수-list
\identifier{poly_mul}/\identifier{matrix_vec_mul}로 전사하는 adapter도 없다.
...
최종 판정은 \identifier{STOP-KG-NTT}다. KG-2/finalization은
...
\identifier{STOP-VERIFY-MATRIX-CRT}이며 첫 재개 leaf는
\identifier{rq_mul_coeff_foldr_to_bigi}, 이어 2-by-4 NTT/CRT 절차 leaf다.
