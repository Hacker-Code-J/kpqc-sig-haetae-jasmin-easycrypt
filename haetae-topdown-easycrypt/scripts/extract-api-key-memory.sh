...
compile_target "$KEYGEN_DIR/KeygenApiCopyTarget.ec" "$KEYGEN_DIR"
compile_target "$SIGN_DIR/SignApiCopyTarget.ec" "$SIGN_DIR"
compile_target "$VERIFY_DIR/VerifyApiCopyTarget.ec" "$VERIFY_DIR"

printf 'PASS focused API key-memory extraction keygen=%s sign=%s verify=%s\n' \
