...
"$JASMIN2EC_BIN" --array-model=barray \
  ...
  -f __verify_hash_mu \
  -f __verify_challenge_absorb \
  "$ROOT_DIR/haetae-ref-jasmin/jasmin/verification_transcript.jazz"

if [ -n "$SERVER_SOCKET" ]; then
  "$EASYCRYPT_BIN" compile -script -no-eco \
    "$SIGN_DIR/SignTranscriptTarget.ec" -I "$SIGN_DIR" \
    -server "$SERVER_SOCKET" -max-provers 1 < /dev/null
  "$EASYCRYPT_BIN" compile -script -no-eco \
    "$VERIFY_DIR/VerifyTranscriptTarget.ec" -I "$VERIFY_DIR" \
    -server "$SERVER_SOCKET" -max-provers 1 < /dev/null
else
  "$EASYCRYPT_BIN" compile -script -no-eco \
    "$SIGN_DIR/SignTranscriptTarget.ec" -I "$SIGN_DIR" < /dev/null
  "$EASYCRYPT_BIN" compile -script -no-eco \
    "$VERIFY_DIR/VerifyTranscriptTarget.ec" -I "$VERIFY_DIR" < /dev/null
fi

printf 'PASS focused transcript extraction sign=%s verify=%s\n' \
  "$SIGN_DIR" "$VERIFY_DIR"
