...
\begin{enumerate}
  ...
  \item actual KeyGen matrix/finalize snapshot에서는 KG-2-like 분해와
        ...
        frozen paper 시점에는 마지막 \identifier{output_row} NTT-word rewrite 뒤의
        full-NTT convolution과 표현 어댑터가 없어
        \identifier{STOP-KG-NTT}로 동결되었다. 이어 Sign은
        \identifier{STOP-SIGN-CHAL-MODE2}, Verify는 실제 2-by-4
        NTT/CRT/freeze 절차가 없는 \FrozenPaperGate 에서 동결되었다
        \cite{week16-sign-report,week16-verify-report}.
        post-freeze에는 순수 convolution, native \(R_q\) 행곱과
        \(R_q\)--HAETAE 표현 운송이 닫혔고 actual \identifier{avec}도 paper
        \(a\)로 해석되었다. 그러나 security-side synthetic ExpandVecA가 actual
        SHAKE128/rejection 의미론 및 paper \(qj\)와 다르므로 현재는
        \CurrentGate 다. 이는 frozen 82-target 판정을 바꾸지 않는다
        \cite{post-freeze-kg-rq-haetae-report,
        post-freeze-kg-actual-avec-qj-report,paper-freeze-artifacts}.
