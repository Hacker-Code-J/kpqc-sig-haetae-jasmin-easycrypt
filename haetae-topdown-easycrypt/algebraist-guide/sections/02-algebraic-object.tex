...
full-invNTT/pointwise word 표현과 정확히 동일시한다. 그러나 그 표현을 native
\(R_q\) negacyclic 곱으로 옮기는 odd-root orthogonality/full-NTT convolution
정리와, 이어지는 보안 모형 integer-list 곱 어댑터는 없다. 따라서 위 합동식을
\(A s=qj\pmod{2q}\)로 이름만 바꾸거나 faithful KG-1/KG-3가 증명되었다고 읽으면
안 된다. 이 경계의 최종 판정은 \identifier{STOP-KG-NTT}다.
\end{caution}
