...
\begin{tikzpicture}[node distance=7mm and 7mm]
  ...
  \node[partialnode, below=of core] (hbz) {production full-HBZ\\wrapper inverse};
  \node[blockednode, below=of hbz] (full) {full signature codec와\\Sign--Verify correctness};
...
\claimid{OBL-RANS-ACTUAL-SUCCESS-WITNESS}는 \statuspartial 이다.
...
\claimid{OBL-RANS-ACTUAL-SUCCESS-WITNESS}와 production full-HBZ wrapper는
\statuspartial 로 남는다.
...
\statuspartial\ (success-conditioned wrapper frame/size transport 미완) \\
actual encoder success witness &
\statuspartial\ (concrete all-6 입력의 성공 도달성과 종료성 부재) \\
