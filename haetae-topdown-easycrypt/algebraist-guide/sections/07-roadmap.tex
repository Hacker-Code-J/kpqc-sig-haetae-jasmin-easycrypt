...
\subsection{현재 단일 우선순위: Verify matrix CRT/freeze leaf}

\SnapshotWeek{}의 최종 gate는 \CurrentGate 다. 첫 의무는
\begin{quote}
\CurrentPriority
\end{quote}
이다. \CurrentGate 는 actual Verify의 helper-local word/control 결과를 보존한
부분 판정이다. V-1/V-2/V-5/V-6, W64 norm gate와 tail trace/mismatch word는
컴파일되었지만 matrix CRT/freeze 의미론이 없어 V-3/V-4 및 전체 predicate는
동결되었다. 그 leaf 뒤에도 full-NTT convolution/adapter, V-6 integer centering,
norm no-wrap, challenge semantics가 남는다.
...
현재 첫 목표는 \identifier{rq_mul_coeff_foldr_to_bigi}이며, 그 다음
\identifier{verify_matrix_ntt_acc_mode2_cols4_correct}와
\identifier{verify_crt_freeze_mode2_word_exact}를 닫아 combined headline을
합성한다.
