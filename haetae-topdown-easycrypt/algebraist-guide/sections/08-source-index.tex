...
KeyGen/Sign 중단 기준과 현재 Verify 전환 \\
...
\subsection{Week 16 MINCORE KeyGen과 KG--NTT 중단 경계}

아래 \WeekSixteenTargetCount 개 파일은 actual matrix helper의 반환 snapshot,
actual finalizer와 마지막 NTT-word rewrite를 다룬다. snapshot을 논문 matrix
product로 재명명하지 않는 것이 이 표의 핵심 경계다.
...
matrix/finalizer harness의 두 active row consequence만 증명;
\identifier{Rq.\&*}나 security-model 곱은 주장하지 않음 \\
같은 파일 &
odd-root orthogonality/full-NTT convolution 및
\identifier{Rq.poly}--security-list adapter의 부재 &
\claimid{OBL-MINCORE-KEYGEN}이 \identifier{STOP-KG-NTT}인 정확한 경계 \\
...
\sourcepath{WEEK16_KG_NTT_MUL_REPORT.md}와 최신 aggregate log에 기록된 전체 실행은
\identifier{RESULT} \identifier{PASS}, \identifier{authored-targets=77},
...
KeyGen snapshot 파일 및 한 NTT boundary 파일이 manifest completeness,
proof-hole/axiom/debug scan과
fresh compile 대상에 포함된다. 이 PASS는 명명된
...
\begin{enumerate}
  ...
  \item \sourcepath{CLAIM_LEDGER.md}에서
        ...
        \claimid{OBL-MINCORE-SIGN} 아홉 행을 비교한다.
...
\begin{enumerate}
  ...
  \item \sourcepath{WEEK16_MINCORE_PLAN.md}\newline
        ...
        \sourcepath{Bridge.ec}를 대조한다.\newline
        KG-2-like proved boundary와 exact missing leaf,
        \identifier{STOP-KG-NTT}를 분리한다.
