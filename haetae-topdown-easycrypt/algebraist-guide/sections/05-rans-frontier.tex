\section{현재 전선: HBZ/rANS를 유한 동역학계(finite dynamical system)로 읽기}
\label{sec:rans-frontier}

\subsection{왜 production full-HBZ wrapper가 지금의 단일 gate인가}
...
되돌리는 간선(edge), actual copy를 사이에 둔 core 합성(composition)은 각각
닫혔다. \CurrentGate 는 세 actual generated 절차의 성공 조건부
역함수(inverse function)가 컴파일되었다는 판정이다. full signature codec으로
가는 현재 단일 우선순위는 \CurrentPriority 이다
\cite{week13-report,rans-core-composition}.
...
이제 남은 간극(gap)은 이 core 정리(theorem)를 production
\identifier{_encode_hb_z1_full}/\identifier{_decode_hb_z1_full}의
prepare/apply·frame·size 의미론(semantics)과 합성하는 wrapper 경계다.
...
도달가능성(reachability)과 종료성(termination)은 여전히 인증하지 않는다.
\end{caution}
...
보이지 않는다. 따라서 \claimid{OBL-RANS-ACTUAL-SUCCESS-WITNESS}는
\statuspartial 이다.
\end{caution}
...
실제 success branch가 도달 가능하다는 EasyCrypt witness도 아직 없다. pure
trace의 concrete witness, canonical all-zero precondition witness 또는 decoder
state의 size-4 witness는 concrete all-6 입력에서 actual encoder가
\(bad=0\)을 반환한다는 success witness를 대신하지 않는다.

\subsection{full HBZ wrapper의 올바른 목표 모양}

actual encoder는 실패를 표현할 수 있으므로, canonical \(h\)만으로 무조건적인
round trip을 선언해서는 안 된다. 현재 필요한 부모 명제(parent proposition)는 적어도
\[
...
\begin{openobligation}[\claimid{OBL-SIG-HBZ-ENCODE-DECODE}]
actual \identifier{_encode_hb_z1_full}과 \identifier{_decode_hb_z1_full}을 직접
열고, 이미 증명된 prepare/apply 역함수(inverse function)와 actual rANS core
정리(theorem)를 wrapper-level size/frame에 맞춰 합성해야 한다. 이 정리(theorem)가
컴파일되기 전에는 두 번째 (h) codec으로 이동하지 않는다.
\end{openobligation}
...
\statuspartial\ (success reachability와 termination 부재) \\
full HBZ encode/decode & \statuspartial\ (production wrapper 합성 미완) \\
\bottomrule
\end{tabularx}
\end{center}
