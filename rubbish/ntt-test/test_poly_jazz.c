...
static void random_poly(poly *p) {
    for (size_t i = 0; i < MLKEM_N; i++) {
        ...
        int32_t x = (rand() % (8 * MLKEM_Q)) - (4 * MLKEM_Q);
...
static void fill_edge_poly(poly *p, int mode) {
    for (size_t i = 0; i < MLKEM_N; i++) {
        ...
        case 2: p->coeffs[i] = MLKEM_Q - 1; break;
        case 3: p->coeffs[i] = -1; break;
        case 4: p->coeffs[i] = MLKEM_Q; break;
        case 5: p->coeffs[i] = -MLKEM_Q; break;
        case 6: p->coeffs[i] = (i & 1) ? (MLKEM_Q - 1) : 0; break;
