...
int main(int argc, const char **argv) {
  ...
  unsigned char *m, *sig, *m1;
  unsigned char pre[256] = {0};
  unsigned char ctx[255] = {0};

  unsigned char rnd[HAETAE_SEEDBYTES];
  unsigned char keygen_seed[HAETAE_SEEDBYTES] = {0};

  size_t mlen, siglen, i;
  size_t ctxlen, prelen;
  unsigned char ctxlen_b;
  int count;
  unsigned char pk[HAETAE_CRYPTO_PUBLICKEYBYTES],
      sk[HAETAE_CRYPTO_SECRETKEYBYTES];
  int ret_val;

  // Create the REQUEST file
  sprintf(fn_req, "PQCsignKAT_%s.req", HAETAE_CRYPTO_ALGNAME);
  if ((fp_req = fopen(fn_req, "w")) == NULL) {
    printf("Couldn't open <%s> for write\n", fn_req);
    return KAT_FILE_OPEN_ERROR;
  }
  sprintf(fn_rsp, "PQCsignKAT_%s.rsp", HAETAE_CRYPTO_ALGNAME);
  ...
  while (1) {
    ...
    sig = (unsigned char *)calloc(HAETAE_CRYPTO_BYTES, sizeof(unsigned char));
    ...
    randombytes(keygen_seed, HAETAE_SEEDBYTES);

    // Generate the public/private keypair
    if ((ret_val = kat_crypto_sign_keypair(pk, sk, keygen_seed)) != 0) {
      printf("crypto_sign_keypair returned <%d>\n", ret_val);
      free(m);
      free(sig);
      return KAT_CRYPTO_FAILURE;
    }
    fprintBstr(fp_rsp, "pk = ", pk, HAETAE_CRYPTO_PUBLICKEYBYTES);
    fprintBstr(fp_rsp, "sk = ", sk, HAETAE_CRYPTO_SECRETKEYBYTES);
    ...
    if (ctxlen > 255) {
      printf("ERROR: invalid ctxlen <%zu>\n", ctxlen);
      free(m);
      free(sig);
      return KAT_DATA_ERROR;
    }
    ...
    if ((ret_val = kat_crypto_sign(sig, &siglen, m, mlen, pre, 1 + ctxlen, rnd,
                                   sk)) != 0) {
    ...
    fprintf(fp_rsp, "siglen = %lu\n", siglen);
    fprintBstr(fp_rsp, "sig = ", sig, siglen);
    fprintf(fp_rsp, "\n");

    if ((ret_val = kat_crypto_sign_verify(sig, siglen, m, mlen, pre, 1 + ctxlen,
                                          pk)) != 0) {
      printf("crypto_sign_verify returned <%d>\n", ret_val);
      free(m);
      free(sig);
      return KAT_CRYPTO_FAILURE;
    }
