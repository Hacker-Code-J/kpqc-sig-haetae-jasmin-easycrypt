...
본문의 최우선 근거는 실제 EasyCrypt 선언이다. 동결 상태는
\sourcepath{CLAIM_LEDGER.md}와 \sourcepath{manifests/paper-artifacts.md}를,
그 이후 상태는 post-freeze 보고서를 우선한다. 그 다음이 추출 매니페스트와
주차별 보고서다. 주차별 문서의
앞부분은 해당 시점의 역사적 스냅샷이므로 현재 상태 판정에는 다음 자료를
우선한다.

\begin{itemize}[leftmargin=1.5em,itemsep=0.1em,topsep=0.3em]
  \raggedright
  \item \sourcepath{manifests/paper-artifacts.md},
        \sourcepath{POST_FREEZE_KG_RQ_}\allowbreak
        \sourcepath{HAETAE_REPORT.md},
        \sourcepath{POST_FREEZE_KG_ACTUAL_}\allowbreak
        \sourcepath{AVEC_QJ_REPORT.md};
...
동결 작성 증명 & \sourcepath{haetae-topdown-easycrypt/easycrypt/} 아래
매니페스트된 82개 EasyCrypt 대상; paper-freeze 직전 82/82와 현재 frozen-suite
82/82가 모두 보존됨 \\
동결 논문 판정 & \identifier{PAPER-FROZEN}; 내부 중단점은
\identifier{STOP-KG-NTT}, \identifier{STOP-SIGN-CHAL-MODE2},
\identifier{STOP-VERIFY-MATRIX-CRT} \\
최신 post-freeze 판정 & \identifier{STOP-KG-AVEC-QJ};
\identifier{OBL-KG-SECURITY-EXPANDVECA-SEMANTICS}가 첫 수리 의무. 순수 NTT/Rq
및 actual-\(a\)/paper-\(qj\) 보조 정리는 동결 매니페스트 밖에서 별도 검증 \\
...
\begin{boundary}[동결 82대상과 post-freeze 보조 증거]
77번째 대상 \sourcepath{Mode2KeygenNttMulBridge.ec} 뒤에 78번째
\sourcepath{Mode2SignAcceptedCore.ec}와 82번째
\sourcepath{Mode2VerifyCoreSequence.ec}가 actual Sign/Verify control boundary를 fresh
compile한다. Verify 수정 전 78대상 전체 완료 실행은
\sourcepath{logs/verify-all-before-week16-verify.log}에 보존된다. 보존된
\sourcepath{logs/verify-all-before-paper-freeze.log}는 마지막 줄
\texttt{RESULT PASS}\newline
\texttt{authored-targets=82 cache=-no-eco}와 SHA-256
\hashsplit{4cd64e5a656be82710bca1410c4d194}{03a3c661d6b91b0319a0ea8f7c91646da}를
가지는 동결 직전 완료 증거다. 기준 커밋 \identifier{e87bcc3}에 추적된
\sourcepath{logs/verify-all-summary.txt}는 paper-freeze audit를 포함해 같은
terminal 결과와 SHA-256
\hashsplit{08ef9639dc73d56dba42d02999d07897}{d29bc8e60aa100a648e9437fd64387ad}를
가진다. 세 \sourcepath{post-freeze/KgActualAvecQj*.ec} 대상은 이 82개에
포함되지 않으며 각각 별도 fresh compile됐다. Hoare 정리의 \statusproved 는
여전히 실제 절차의 종료나 확률 1 실행을 뜻하지 않는다.
\end{boundary}
...
동결 주장 행렬 안에서의 full-NTT/논문 곱 동일시, 보안 모델 ExpandVecA와 실제
\identifier{avec}의 동일시, 논문 KeyGen 식, MINCORE Sign/Verify 합성, 인코딩
무손실, 또는 Jasmin 구현의
