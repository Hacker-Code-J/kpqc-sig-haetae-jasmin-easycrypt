...
| OBL-FFT-SAFE-TRACE | First-attempt rounded FFT endpoint versus accumulator and retry traces | TO_PROVE | The scheduled FFT safety premises, signed word bounds, and owner-stage error recurrence are discharged under `fft_coefficient_bound xp 2`; the peeled first attempt establishes that predicate for all five slices, whose outputs are within `44833/65536` of `odd_dft256`. Decoded squared magnitude and the five-slice ideal-energy error are propagated under the exact evolving `fft_accumulate_safe` trace. A conservative ideal prefix-energy and coordinate-headroom condition implies that trace, and an axiom-free schema bounds its first-attempt failure probability by `1536 * (delta_lower + delta_upper) + 1280 * (delta_real + delta_imag)` from uniform per-site marginal bounds. Supply a checked keygen first-attempt distribution and instantiate those four local tails, propagate the input/headroom/error facts and probability accounting to later attempts, and connect energy to the tie-sensitive score and guard before claiming full retry semantics. |
