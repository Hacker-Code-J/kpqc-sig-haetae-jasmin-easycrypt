...
| OBL-FFT-SAFE-TRACE | First-attempt rounded FFT endpoint versus accumulator and retry traces | TO_PROVE | The scheduled safety premises, signed FFT word bounds, and owner-stage error recurrence are discharged under `fft_coefficient_bound xp 2`; the actual peeled first attempt establishes that predicate for all three `s1` and two finalized-`s2` slices, whose outputs are within `44833/65536` of `odd_dft256`. Establish squared-magnitude/accumulator nonoverflow, propagate the input and error facts to later attempts, and connect the score and guard before claiming full retry semantics. |
