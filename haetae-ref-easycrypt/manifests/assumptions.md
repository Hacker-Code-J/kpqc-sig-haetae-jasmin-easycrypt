...
| OBL-FFT-SAFE-TRACE | First-attempt safe rounded FFT versus the remaining ideal/global-error and accumulator trace | TO_PROVE | The scheduled `fft_butterfly_safe_at` premises and signed FFT word bounds are discharged under `fft_coefficient_bound xp 2`; the actual peeled first attempt establishes that predicate for all three `s1` and two finalized-`s2` slices, and the recurrence through endpoint budget `44833/65536` is checked. Lift the budget through the owner-stage observer against the ideal schedule, establish squared-magnitude/accumulator nonoverflow, and propagate the input facts to later attempts before using the score for full retry semantics. |
