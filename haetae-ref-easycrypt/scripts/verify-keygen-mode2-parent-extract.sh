...
EXTRACT_DIR="$PROJECT_DIR/easycrypt/extract/keygen-mode2-parent"
...
LOG_DIR="$PROJECT_DIR/logs/keygen-mode2-parent-extract"
SUMMARY="$PROJECT_DIR/logs/keygen-mode2-parent-extract-summary.txt"
...
{
  printf 'HAETAE deterministic mode-2 key-generation parent extraction verification\n'
  printf 'Date (UTC): %s\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')"
  printf 'Source: ../haetae-ref-jasmin/jasmin/keypair.jazz\n'
  printf 'Selected function: crypto_sign_keypair_internal_mode2_jazz\n'
  printf 'Extraction model: barray\n'
  printf 'Generated file manifest: manifests/keygen-mode2-parent-extract-files.txt\n'
  printf 'Procedure closure manifest: manifests/keygen-mode2-parent-procedures.txt\n'
...
"$SCRIPT_DIR/check-keygen-mode2-parent-extract-drift.sh" | tee -a "$SUMMARY"
...
sha256sum "$EXTRACT_DIR/KeygenMode2ParentTarget.ec" >> "$SUMMARY"
