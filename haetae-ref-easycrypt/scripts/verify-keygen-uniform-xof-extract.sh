...
EXTRACT_DIR="$PROJECT_DIR/easycrypt/extract/keygen-uniform-xof"
...
LOG_DIR="$PROJECT_DIR/logs/keygen-uniform-xof-extract"
SUMMARY="$PROJECT_DIR/logs/keygen-uniform-xof-extract-summary.txt"
...
{
  printf 'HAETAE mode-2 key-generation uniform XOF leaf extraction verification\n'
  printf 'Date (UTC): %s\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')"
  printf 'Source: ../haetae-ref-jasmin/jasmin/keypair.jazz\n'
  printf 'Selected functions: _kp_poly_uniform_at_seedbuf_8192, _kp_poly_uniform_at_seedbuf_2048\n'
  printf 'Extraction model: barray\n'
  printf 'Generated file manifest: manifests/keygen-uniform-xof-extract-files.txt\n'
  printf 'Procedure closure manifest: manifests/keygen-uniform-xof-procedures.txt\n'
...
"$SCRIPT_DIR/check-keygen-uniform-xof-extract-drift.sh" | tee -a "$SUMMARY"
...
sha256sum "$EXTRACT_DIR/KeygenUniformXofTarget.ec" >> "$SUMMARY"
