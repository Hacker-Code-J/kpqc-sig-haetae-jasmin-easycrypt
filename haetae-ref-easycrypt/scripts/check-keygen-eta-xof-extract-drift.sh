...
OUT_DIR="$PROJECT_DIR/easycrypt/extract/keygen-eta-xof"
...
jasmin2ec --array-model=barray \
  ...
  -o "$TMP_DIR/KeygenEtaXofTarget.ec" \
...
sed -n 's/^[[:space:]]*proc \([^ (]*\).*/\1/p' \
  "$TMP_DIR/KeygenEtaXofTarget.ec" > "$TMP_DIR/generated-procedures.txt"
...
printf 'PASS: key-generation eta XOF extraction has zero regeneration drift (%s files, %s procedures)\n' \
