...
EXTRACT_DIR="$PROJECT_DIR/easycrypt/extract/keygen-sampler-callers"
FILE_MANIFEST="$PROJECT_DIR/manifests/keygen-sampler-callers-extract-files.txt"
PROCEDURE_MANIFEST="$PROJECT_DIR/manifests/keygen-sampler-callers-procedures.txt"
LOG_DIR="$PROJECT_DIR/logs/keygen-sampler-callers-extract"
SUMMARY="$PROJECT_DIR/logs/keygen-sampler-callers-extract-summary.txt"
...
{
  printf 'HAETAE key-generation sampler leaf-plus-caller extraction verification\n'
  printf 'Date (UTC): %s\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')"
  printf 'Source: ../haetae-ref-jasmin/jasmin/keypair.jazz\n'
  printf 'Selected functions: _kp_expand_seedbuf, _kp_polymatkm_expand_matA, _kp_polyveck_expand_vecA, _kp_polyvec_expand_eta\n'
  printf 'Extraction model: barray\n'
  printf 'Generated file manifest: manifests/keygen-sampler-callers-extract-files.txt\n'
  printf 'Procedure closure manifest: manifests/keygen-sampler-callers-procedures.txt\n'
...
"$SCRIPT_DIR/check-keygen-sampler-callers-extract-drift.sh" | tee -a "$SUMMARY"
...
sha256sum "$EXTRACT_DIR/KeygenSamplerCallersTarget.ec" >> "$SUMMARY"
