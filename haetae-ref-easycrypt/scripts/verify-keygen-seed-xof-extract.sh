...
EXTRACT_DIR="$PROJECT_DIR/easycrypt/extract/keygen-seed-xof"
FILE_MANIFEST="$PROJECT_DIR/manifests/keygen-seed-xof-extract-files.txt"
PROCEDURE_MANIFEST="$PROJECT_DIR/manifests/keygen-seed-xof-procedures.txt"
LOG_DIR="$PROJECT_DIR/logs/keygen-seed-xof-extract"
SUMMARY="$PROJECT_DIR/logs/keygen-seed-xof-extract-summary.txt"
...
{
  printf 'HAETAE mode-2 key-generation seed XOF extraction verification\n'
  printf 'Date (UTC): %s\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')"
  printf 'Source: ../haetae-ref-jasmin/jasmin/keypair.jazz\n'
  printf 'Selected function: _kp_expand_seedbuf\n'
  printf 'Extraction model: barray\n'
  printf 'Generated file manifest: manifests/keygen-seed-xof-extract-files.txt\n'
  printf 'Procedure closure manifest: manifests/keygen-seed-xof-procedures.txt\n'
...
"$SCRIPT_DIR/check-keygen-seed-xof-extract-drift.sh" | tee -a "$SUMMARY"
...
sha256sum "$EXTRACT_DIR/KeygenSeedXofTarget.ec" >> "$SUMMARY"
