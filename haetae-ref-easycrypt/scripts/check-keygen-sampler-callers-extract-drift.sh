...
OUT_DIR="$PROJECT_DIR/easycrypt/extract/keygen-sampler-callers"
...
jasmin2ec --array-model=barray \
  ...
  -o "$TMP_DIR/KeygenSamplerCallersTarget.ec" \
...
sed -n 's/^[[:space:]]*proc \([^ (]*\).*/\1/p' \
  "$TMP_DIR/KeygenSamplerCallersTarget.ec" > "$TMP_DIR/generated-procedures.txt"
...
printf 'PASS: key-generation sampler-caller extraction has zero regeneration drift (%s files, %s procedures)\n' \
