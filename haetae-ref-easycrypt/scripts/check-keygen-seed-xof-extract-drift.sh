...
OUT_DIR="$PROJECT_DIR/easycrypt/extract/keygen-seed-xof"
...
jasmin2ec --array-model=barray \
  ...
  -o "$TMP_DIR/KeygenSeedXofTarget.ec" \
...
sed -n 's/^[[:space:]]*proc \([^ (]*\).*/\1/p' \
  "$TMP_DIR/KeygenSeedXofTarget.ec" > "$TMP_DIR/generated-procedures.txt"
...
printf 'PASS: key-generation seed XOF extraction has zero regeneration drift (%s files, %s procedures)\n' \
